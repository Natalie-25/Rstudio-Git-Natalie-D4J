...
To remove NA vakues from the hdi data and widen the bars
...
order_2023 <- southern_compare %>%
  filter(year == 2023) %>%
  distinct(country, .keep_all = TRUE) %>%   # prevents duplicates
  arrange(desc(hdi)) %>%
  pull(country)
...
Evening out the width of the bars for both years for better comparison
...
ggplot(
  ...
  # Extra wide bars for BOTH years
  geom_col(
