...
To clean the variables headines in the dataset
...
Add HDI labels to the graph for each period


southern_compare <- southern_dta %>%
  filter(year %in% c(1990, 2023))

ggplot(southern_compare, aes(x = reorder(country, hdi), y = hdi, fill = factor(year))) +
  
  Base bars for all countries
  geom_col(position = position_dodge(width = 0.9)) +
  
  Highlight Mozambique bars
  geom_col(
    data = southern_compare %>% filter(country == "Mozambique"),
    aes(x = reorder(country, hdi), y = hdi),
    fill = "#ffa600",
...
the hdi values for 1990 are not properly scaled and so this needs to be corrected
...
Clean the bar further so that there is clear contrast between years 1990 and 2023
...
Making the bar chart look nicer
...
To remove NA and widen the bars
...
Developing an interactive map_data
...
# Remove missing HDI values (recommended)
